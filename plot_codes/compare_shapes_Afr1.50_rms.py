...
DH_ax.set_title('Double-horn $A_{fr}$ = 1.5', fontsize=18)
GA_ax.set_title('Gaussian $A_{fr}$ = 1.5', fontsize=18)

DH_ax.set_ylim(0.99,2.25)
...
for ii in range(len(bases)):
	...
	ax.plot(sym_stats['avg_SN_w20'],0.5 + sym_stats['P50_w20'], color = 'Blue', ls = '-',zorder = -1,lw=2,label='0.5 + 50$^{th}$ symmetric percentile')
	ax.plot(sym_stats['avg_SN_w20'],0.5 + sym_stats['P90_w20'], color = 'Red', ls = '-',zorder = 0,lw=2,label='0.5 + 90$^{th}$ symmetric percentile')

	asym_filename = '{base}AAVsm10_Afr1.50_rms/statistics.dat'.format(base=base)
	...
	ax.plot([5,102],[1.5,1.5],color='Grey')
...
plotdir = 'figures/asymcompare_Afr1.50_rms.png'.format(base = base)
